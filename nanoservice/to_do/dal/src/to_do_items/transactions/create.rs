...
/// About the trait SaveOne.
/// If we were to just use an async function in our trait,
/// then we would not be explicit about whether the Send trait needs to be implemented or not for the future.
/// This means that we can use our future in multithreaded contexts, which is our Tokio runtime.
/// Tokio is merely an implementation of an async runtime.
pub trait SaveOne {
    fn save_one(
        item: NewToDoItem,
